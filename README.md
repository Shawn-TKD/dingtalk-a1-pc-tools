...
- 读取设备状态和录音索引
- 只读查询灰度配置与电量命令
- 下载指定录音；设备内录音可用输入确认码删除，未备份时使用更严格的 `NOBACKUP` 确认码
...
`start-a1-console.cmd`。启动窗口会生成一次性的随机访问令牌，同时显示电脑和
手机访问地址。手机与电脑必须处在同一个可信局域网。
...
设备索引中的录音都可以删除。已有 `.dtyj` 与 `.ogg` 双备份时，确认码为
`DELETE-<fid>`，并且只删除 A1 内文件；没有备份时，页面会明确提示无法恢复，确认码
升级为 `DELETE-NOBACKUP-<fid>`。设备删除后，已有备份仍列为“仅本地”并支持播放。
