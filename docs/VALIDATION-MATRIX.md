...
| `0x0113` single-file delete | Confirmed live on one owned device + guarded implementation | The decompiled helper uses string fields `did` and `fid`. One request returned `code:200` and the next read showed one fewer indexed recording; existing local backup files remained. The console requires the fid to be in the current device index and one explicit confirmation dialog; unbacked files show a stronger irreversible-loss warning. Automated tests mock the device and never send this command. |
